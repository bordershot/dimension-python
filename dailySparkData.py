...
def main():
    parser = argparse.ArgumentParser(
        description='Collect SparkEV data from voltstats')
    parser.add_argument('password', help='voltstats password')
    parser.add_argument('--all', action="store_true",
                        default=False, help='Upload all data for all time')
    args = parser.parse_args()

    data_type_dict = {'Download Reading Data': {'label': 'spark.miles',
                                                'daily_line': 1, 'field': 10},
                      'Download Daily Data': {'label': 'spark.dailymiles',
                                              'daily_line': -2, 'field': 1}}
    graphite_message = ""
    graphite_server = 'grafana-vm'
...
#        print(r.text)
        ...
        if not args.all:
            reading_list = [reading_list[
                data_type_dict[data_type]['daily_line']]]
        else:
...
#        print(reading_list.split(','))
        for reading in reading_list:
            today = reading.split(',')
#            print(reading)
            graphite_date = int(time.mktime(
                time.strptime(today[0], '"%m/%d/%Y %H:%M:%S %p"')))
            data = re.search(r'"(\S+)"',
                             today[data_type_dict[data_type]['field']])
            graphite_message = (graphite_message +
                                data_type_dict[data_type]['label'] + ' ' +
                                data.group(1) + ' ' + str(graphite_date) +
                                '\n')
